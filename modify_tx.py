...
new_code = """import { useState, useEffect, useMemo } from 'react';
...
export function TransactionsPage() {
  ...
  const filteredAndSorted = useMemo(() => {
    ...
    if (toDate) {
      ...
      result = result.filter(t => new Date(t.transactionDate || t.receivedAt).getTime() <= to + 86400000); // include end of day
    }
  ...
  return (
    <div className="p-4 md:p-8">
      <div className="max-w-[1400px] mx-auto space-y-6">
        ...
        {/* Filters & Controls */}
        <div className="glass rounded-3xl p-6 mb-6 border border-white/10">
          <div className="flex flex-col xl:flex-row gap-4 items-end">
            ...
            <div className="flex flex-wrap gap-4 w-full xl:w-auto">
              ...
              <div className="flex items-center gap-2 bg-white/50 dark:bg-black/20 border border-border rounded-xl px-3 py-1.5 focus-within:ring-2 focus-within:ring-primary/20 transition-all">
            ...
            <div className="flex-1 w-full min-h-0">
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={chartData} margin={{ top: 10, right: 10, left: 0, bottom: 0 }}>
                  ...
                  />
                  <Tooltip 
                    contentStyle={{ borderRadius: '1rem', backgroundColor: 'var(--tw-prose-body)', border: 'none', color: '#fff' }}
                      ...
                      <tr key={tx.id} className={`transition-colors ${isEditing ? 'bg-primary/5 dark:bg-primary/10' : 'hover:bg-white/30 dark:hover:bg-black/10'}`}>
                        ...
                        <td className="p-5 text-right align-top">
                          {isEditing ? (
                            ...
                            <>
                              <div className={`text-sm font-black flex items-center justify-end gap-1 ${isDebit ? 'text-foreground' : 'text-emerald-600 dark:text-emerald-400'}`}>
...
with open('src/pages/TransactionsPage.tsx', 'w') as f:
    f.write(new_code)
